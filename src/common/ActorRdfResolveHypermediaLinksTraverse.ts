...
export class ActorRdfResolveHypermediaLinksTraverse extends ActorRdfResolveHypermediaLinks {
    public static readonly CONTEXT_KEY_TRAVERSE = new ActionContextKey<boolean>(
        '@comunica/actor-rdf-resolve-hypermedia-links-traverse:traverse',
    );
    ...
    public async test(action: IActionRdfResolveHypermediaLinks): Promise<IActorTest> {
        ...
        if (
            action.context.has(ActorRdfResolveHypermediaLinksTraverse.CONTEXT_KEY_TRAVERSE) &&
